    ...
    public func kMeansCluster(colors: [Color], clusters: Int, iterations: Int = 10) -> [Color] {
        ...
        for _ in 0 ..< iterations {
            ...
            centroids = clusterArray.map { cluster in
                ...
                let red = cluster.map(\.redValue).reduce(0, +) / Double(cluster.count)
                let green = cluster.map(\.greenValue).reduce(0, +) / Double(cluster.count)
                let blue = cluster.map(\.blueValue).reduce(0, +) / Double(cluster.count)
