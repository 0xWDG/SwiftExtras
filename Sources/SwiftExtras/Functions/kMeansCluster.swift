...
///
/// This function takes an array of colors and clusters them into a specified
/// number of groups.
/// It uses the K-Means algorithm to find the centroids of the clusters.
/// The algorithm iteratively assigns colors to the nearest centroid and then
...
/// - Note: This implementation uses the Euclidean distance in RGB space to
/// determine the distance between colors.
public func kMeansCluster(
    colors: [Color],
    clusters: Int,
    iterations: Int = 10
) -> [Color] {
    ...
    for _ in 0 ..< iterations {
        ...
        for color in colors {
            ...
            guard let distance = distances.enumerated()
        ...
        centroids = clusterArray.map { cluster in
            ...
            let red = cluster.map(\.redValue)
                .reduce(0, +) / Double(cluster.count)
            let green = cluster.map(\.greenValue)
                .reduce(0, +) / Double(cluster.count)
            let blue = cluster.map(\.blueValue)
                .reduce(0, +) / Double(cluster.count)
