...
// The confetti is displayed in a container view that manages the animation and
// positioning of the confetti shapes.
// The confetti shapes can be customized, and the colors can be set to a variety
// of vibrant colors.
// The confetti animation can be triggered by setting a binding variable to
// true, \
// and it will automatically fade out after a specified duration.
// The confetti shapes can be any SwiftUI view, allowing for flexibility in
// design.
...
struct ConfettiView<ConfettiShape: View>: View {
    ...
    /// The size of the view, used to determine the bounds for positioning
    /// confetti pieces.
    /// This is initialized to a large size to ensure confetti can be positioned
    ...
    /// - Parameters:
    ///   - colors: An array of colors for the confetti pieces. Defaults to a
    /// set of vibrant colors.
    ///   - shape: The shape of the confetti pieces, which can be any SwiftUI
    /// view. Defaults to a rectangle.
            ...
            ForEach(0 ..< count, id: \.self) { _ in
                Confetti(colors: colors, shape: shape)
                    .position(
                        ...
                        y: yPosition != 0 ? CGFloat
                            .random(in: 0 ... viewSize.height) : yPosition
                    )
            }
        }
    ...
    ///   - shape: The shape of the confetti piece, which can be any SwiftUI
    /// view.
    ...
    struct Confetti: View {
        ...
        /// It starts as false and becomes true when the view appears,
        ...
        /// This is a random value between 1 and 2 seconds, which determines how
        /// fast
        /// the confetti rotates around the z-axis.
        @State private var zSpeed = Double.random(in: 1 ... 2)

        /// The anchor point for the z-axis rotation, which is a random value
        /// between 0 and 1.
        /// This value is rounded to ensure it is a valid anchor point.
        /// It determines the point around which the confetti rotates in 3D
        /// space.
        @State private var anchor = CGFloat.random(in: 0 ... 1).rounded()

        /// The colors of the confetti piece, which are randomly selected from
        /// the provided array.
        ...
        /// Initializes a new `Confetti` view with the specified colors and
        /// shape.
        /// - Parameters:
        ///   - colors: An array of colors for the confetti piece.
        ///   - shape: The shape of the confetti piece, which can be any SwiftUI
...
struct ConfettiModifier<ConfettiShape: View>: ViewModifier {
    ...
    /// A flag that determines whether the confetti animation should
    /// automatically end after a set duration.
    var automaticEnd: Bool = true

    /// The shape of the confetti pieces, which can be any SwiftUI view.
    var shape: ConfettiShape

    /// The opacity of the confetti animation, which starts at 1.0 and fades out
    /// to 0.
    ...
    func body(content: Content) -> some View {
        if #available(iOS 17.0, macOS 14.0, *) {
            content
                .overlay(
                    isActive
                        ? ConfettiView(colors: colors, shape: shape)
                        .opacity(opacity)
                        : nil
                )
    ...
    private func handleAnimationSequence() async {
        ...
        do {
            try await Task
                .sleep(nanoseconds: UInt64(animationTime * 1_000_000_000))
...
public extension View {
    ...
    ///   - colors: An array of colors for the confetti. Defaults to a set of
    /// vibrant colors.
    ///   - automaticEnd: A boolean that determines whether the confetti
    /// animation should \
    ///                   automatically end after a set duration. Defaults to
    /// true.
    ///   - shape: The shape of the confetti. Defaults to a rectangle.
    ///   - animationTime: The duration of the confetti animation in seconds.
    ///   - fadeTime: The duration of the fade-out effect after the confetti
    /// animation ends.
