...
/// IndexedList is a view that displays a list of items grouped by their initial
/// letter.
...
/// The list is scrollable and each item can be customized using a cell builder
/// closure.
public struct IndexedList<Cell: View>: View {
    ...
    ///     view of type `Cell` to be used as the content for each row in the
    /// list.
    /// - Example:
    /// ```swift
    ...
    public init(
        ...
        // Grouping the data by the first letter of each string, converting it
        // to uppercase
        // This creates a dictionary where the keys are the first letters and
        // the values are arrays of strings
        // This allows for quick access to items based on their initial letter
        // The keys are sorted alphabetically to maintain a consistent order in
        // the list
    ...
    public var body: some View {
        ...
        ScrollViewReader { proxy in
            // The main ZStack contains the background color, the List, and the
            // indexed letter column
            ZStack(alignment: .topTrailing) {
                ...
                List {
                    // Using ForEach to iterate over the sorted keys of the
                    // listData dictionary
                    ForEach(sortedKeys, id: \.self) { key in
                        // Unwrapping the items for the current key
                        if let items = listData[key] {
                            // For each key, we create a section with the header
                            // as the key
                            Section(header: Text(key).id(key)) {
                                // Using ForEach to iterate over the items in
                                // each section
                                ForEach(items, id: \.self) { name in
                                    // Building the cell using the provided cell
                                    // builder closure
                                    cellBuilder(name)
                                }
                            }
                ...
                // Add extra padding to the list to avoid clipping the indexed
                // letter column
                .padding(.trailing, 10)
                ...
                // Using a GeometryReader to create a vertical column of indexed
                // letters
                // This column allows users to quickly navigate to sections of
                // the list
                GeometryReader { geo in
                    ...
                    VStack(spacing: 8) {
                        ...
                        ForEach(sortedKeys, id: \.self) { key in
                            ...
                            Text(key)
                                ...
                                // Add some padding
                                .padding(2)
                                // On tap gesture to scroll to the section
                                ...
                                .background(
                                    // Using GeometryReader to capture the frame
                                    // of each index key
                                ...
                                // Accessibility traits to make it clear that
                                // this is a link
                    ...
                    // This is particularly useful for iOS where scrubbing is
                    // common
                    // The drag gesture updates the current index based on the
                    ...
                    .gesture(
                        ...
                        DragGesture(minimumDistance: 1)
                            .onChanged { value in
                                // Check if the current index is nil or if it
                                // has changed
                                if let match = indexForLocation(
                                    ...
                                    // If the current index is nil or has
                                    // changed, \
                ...
                // Add a preference key to capture the frames of the indexed
                // letters
                .onPreferenceChange(IndexKeyPreferenceKey.self) { values in
                    indexKeyFrames = values
                }
                // Set the frame width of the indexed letter column
                // This ensures that the column has a fixed width for
                // consistency
    ...
    /// - Returns: A `Color` that represents the background color of the
    /// IndexedList view.
    ...
    /// Determines the index key for a given location in the indexed letter
    /// column.
    /// This function checks if the location falls within the frame of any
    /// indexed letter.
    /// - Parameters:
    ///   - location: The CGPoint representing the location of the gesture.
    ///   - container: The CGRect representing the frame of the indexed letter
    /// column.
    ///   - keyFrames: An array of `IndexKeyInfo` containing the keys and their
    /// corresponding frames.
    /// - Returns: The key of the indexed letter that corresponds to the
    /// location, or `nil` if no match is found.
    private func indexForLocation(
        _ location: CGPoint,
        in container: CGRect,
        keyFrames: [IndexKeyInfo]
    ) -> String? {
        // Check if the location is within the bounds of the indexed letter
        // column
        for info in keyFrames where info.frame.contains(CGPoint(
            x: container.midX,
            y: location.y
        )) {
            return info.key
...
/// This is used to capture the frames of the indexed letters for scrubbing
/// functionality.
...
/// This is used to store the frames of the indexed letters in the IndexedList
/// view.
/// It conforms to `PreferenceKey` to allow the IndexedList view to update its
/// state based \
/// on the frames of the indexed letters.
struct IndexKeyPreferenceKey: PreferenceKey {
    static var defaultValue: [IndexKeyInfo] = []
    static func reduce(
        value: inout [IndexKeyInfo],
        nextValue: () -> [IndexKeyInfo]
    ) {
...
#Preview {
    IndexedList(data: [
        "Wesley", "Carlo", "Uneata", "Anne", "Bram", "Sanne", "Daan", "Lieke",
        "Milan",
        "Femke", "Joris", "Tess", "Thijs", "Noa", "Sem", "Lars", "Lotte", "Max",
        "Eva",
        "Luuk", "Nina", "Finn", "Roos", "Janneke", "Gijs", "Isa", "Koen",
        "Sofie",
        "Tijn",
        "Maud", "Ruben", "Evi", "Siem", "Luca", "Bo", "Jelle", "Fleur", "Mees",
        "Yara",
        "Pim", "Elin", "Stijn", "Mare", "Noud", "Saar", "Tim", "Bente",
        "Jochem",
        "Ilse",
        "Pepijn", "Marit", "Teun", "Milou", "Jip", "Jet", "Bas", "Anouk",
        "Timo",
        "Veerle",
        "Floris", "Lana", "Jens", "Mila", "Job", "Loes", "Cas", "Tirza", "Nick",
        "Fenna",
        "Hidde", "Lynn", "Dirk", "Jade", "Mark", "Iris", "Bart", "Elise",
        "Wout",
        "Norah",
        "Maarten", "Nora", "Kees", "Tessa", "Rik", "Amber", "Nathan", "Vera",
        "Roel",
        "Zara",
        "Jan", "Esmee", "Tom", "Britt", "Stef", "Demi", "Arjen", "Floor",
        "Johan",
        "Liv",
