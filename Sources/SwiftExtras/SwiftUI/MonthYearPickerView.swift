...
/// This view provides a picker for selecting a month and a year, with options
/// to limit the selection range.
...
public struct MonthYearPickerView: View {
    ...
    ///   - minimumDate: The earliest date that can be selected (default is
    /// 1970-01-01).
    ///   - maximumDate: The latest date that can be selected (default is
    /// distant future).
    public init(
        ...
        months = Calendar.current.monthSymbols.map(\.capitalized)
...
@available(iOS 17.0, macOS 14.0, tvOS 17.0, watchOS 10.0, *)
#Preview {
    @Previewable @State var selectedMonth = 1
    @Previewable @State var selectedYear = 2021
    MonthYearPickerView(
