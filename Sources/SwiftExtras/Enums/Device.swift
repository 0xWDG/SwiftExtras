...
public enum Device {
    ...
    public static var model: String {
        ...
        let optionalString: String? = withUnsafePointer(to: &utsnameInstance
            .machine
        ) {
    ...
    var isCarplay: Bool {
        ...
        if #available(iOS 16, *) {
            return UIApplication.shared.connectedScenes.filter {
                ($0 as? UIWindowScene)?.traitCollection
    ...
    static var deviceIdentifierForVendor: String {
        UIDevice.current.identifierForVendor?.uuidString ?? "No ID"
    }
    ...
    static var deviceBatteryState: UIDevice
