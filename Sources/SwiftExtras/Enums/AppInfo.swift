...
/// This can be used to get the application name, version number, build number,
/// etc.
public enum AppInfo {
    ...
    public static var isTestflight: Bool {
        Bundle.main.appStoreReceiptURL?.absoluteString
            .contains("sandboxReceipt") ?? false
    }
    ...
    public static func appStoreInfo(_ forceRefresh: Bool = false) async
        -> SEAppInfoAppStoreInfo? {
        #if os(macOS) || os(iOS)
        let decoder = JSONDecoder()

        if let cached = UserDefaults.standard
            .data(forKey: "SEAppInfoAppStoreInfo"),
            let appStoreInfo = try? decoder.decode(
                SEAppInfoAppStoreInfo.self,
                from: cached
            ),
            !forceRefresh {
            return appStoreInfo
    ...
    public static func getReviewURL() async -> URL? {
        if let identifier = await AppInfo.appStoreInfo()?.results.first?
            .trackId,
            let url = URL(
    ...
    public static func getDeveloperURL() async -> URL? {
        if let identifier = await AppInfo.appStoreInfo()?.results.first?
            .artistId,
            let url =
            URL(string: "https://apps.apple.com/developer/id\(identifier)") {
            return url
        }
    ...
    public static func isLatestVersion() async -> Bool? {
        if let version = await AppInfo.appStoreInfo(true)?.results.first?
            .version {
    ...
    public static var appIcon: Image {
        #if canImport(UIKit)
        guard let icons = Bundle.main
            .object(forInfoDictionaryKey: "CFBundleIcons")
            as? [String: Any],
        ...
        guard let uiImage = UIImage(named: iconFileName)
        else {
            return Image(systemName: "xmark.app")
        }
        ...
        guard let iconFileName = Bundle.main
            .object(forInfoDictionaryKey: "CFBundleIconName")
            as? String
        else {
            return Image(systemName: "xmark.app")
        }

        guard let nsImage = NSImage(named: iconFileName)
        else {
            return Image(systemName: "xmark.app")
        }
    ...
    public static var deviceType: Image {
        #if os(iOS)
        Image(systemName: UIDevice.current
            .userInterfaceIdiom == .pad ? "ipad" : "iphone"
        )
        #elseif os(macOS)
