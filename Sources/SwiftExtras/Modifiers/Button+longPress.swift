...
/// This style allows you to create a button that responds to long press
/// gestures.
/// It scales down the button when pressed and executes the provided action on
/// long press.
struct LongPressButtonStyle: PrimitiveButtonStyle {
    ...
    /// ```swift
    /// Text("Long Press Me")
    ///     .buttonStyle(LongPressButtonStyle(longPressAction: { print("Long
    /// pressed!") }))
    /// ```
...
struct LongPressModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .buttonStyle(LongPressButtonStyle(longPressAction: longPressAction))
    }
}
