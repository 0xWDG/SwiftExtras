...
public extension Locale {
    ...
    static var userLanguage: String {
        current.language.languageCode?.identifier ?? "Unknown"
    }

    /// The user's currency code (e.g., "USD")
    static var userCurrencyCode: String {
        current.currency?.identifier ?? "Unknown"
    }
    ...
    static var deviceMeasurementSystem: Locale
    ...
    static var userCalendar: String {
        String(describing: current.calendar.identifier)
    }
