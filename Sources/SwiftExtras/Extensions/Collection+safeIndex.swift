...
public extension Collection {
    ...
    ///
    /// This subscript provides a safe way to access elements within the
    /// collection by
    /// returning `nil` instead of crashing when accessing an index out of
    /// bounds.
    /// It is useful for avoiding runtime errors when the exact count of the
    /// collection
    ...
    /// range simply result in `nil`, allowing for more graceful handling of
    /// such cases.
    ///
    /// - Parameter index: The position of the element to access.
    ///                    If the index is within the bounds of the collection,
    /// \
    ///                    the element at that position is returned. Otherwise,
    /// `nil` is returned.
    /// - Returns: The element at the specified index if it is within the
    /// collection's bounds; otherwise, `nil`.
    ///
    /// Example usage:
    /// ```
